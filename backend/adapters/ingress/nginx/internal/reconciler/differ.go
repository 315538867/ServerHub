...
func buildInspectScript(p render.Profile) string {
	return `set -eu
...
func Inspect(r runner.Runner) (map[string]ActualFile, error) {
	return InspectWith(r, render.DefaultProfile())
}

// InspectWith 同 Inspect，但允许调用方指定 Profile（多实例必需）。
func InspectWith(r runner.Runner, p render.Profile) (map[string]ActualFile, error) {
...
func Diff(desired []render.ConfigFile, actual map[string]ActualFile) []Change {
	desiredMap := make(map[string]render.ConfigFile, len(desired))
