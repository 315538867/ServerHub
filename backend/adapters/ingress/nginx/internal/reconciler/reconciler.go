...
//   4. LoadProfile：DB → render.Profile（多实例路径/命令）
//   5. LoadDesired：DB → IngressCtx
//   6. render.RenderWith → desired ConfigFile
...
func Apply(ctx context.Context, db *gorm.DB, cfg *config.Config, edgeID uint, actor *uint) (ApplyResult, error) {
	...
	desired, err := render.RenderWith(desiredCtxs, profile)
	if err != nil {
...
func syncCerts(r runner.Runner, ctxs []render.IngressCtx) error {
...
func rollback(r runner.Runner, changes []Change) error {
	return rollbackWith(r, changes, render.DefaultProfile())
}

// rollbackWith 按 Diff 反向把远端还原到 apply 前。失败合并错误返回。
// Profile 用于 sites-enabled symlink 修复，多实例必需。
func rollbackWith(r runner.Runner, changes []Change, p render.Profile) error {
...
func syncSitesEnabled(r runner.Runner, desired []render.ConfigFile) error {
	return syncSitesEnabledWith(r, desired, render.DefaultProfile())
}
...
func syncSitesEnabledWith(r runner.Runner, desired []render.ConfigFile, p render.Profile) error {
	p = render.NormalizeProfile(p)
	var enable []string
...
func syncSitesEnabledFromRemote(r runner.Runner) error {
	return syncSitesEnabledFromRemoteWith(r, render.DefaultProfile())
}

// syncSitesEnabledFromRemoteWith 在 rollback 之后再扫一次远端 sites-available 修复 symlink。
func syncSitesEnabledFromRemoteWith(r runner.Runner, p render.Profile) error {
	p = render.NormalizeProfile(p)
