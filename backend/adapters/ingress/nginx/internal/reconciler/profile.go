...
func LoadProfile(db *gorm.DB, edgeID uint) (render.Profile, *model.NginxProfile, error) {
	var np model.NginxProfile
	err := db.Where("edge_server_id = ?", edgeID).First(&np).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return render.DefaultProfile(), nil, nil
	}
	if err != nil {
		return render.Profile{}, nil, err
	}
	rp := render.NormalizeProfile(render.Profile{
