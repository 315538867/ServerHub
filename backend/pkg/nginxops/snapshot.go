...
func Snapshot(r runner.Runner, edgeID uint) (string, error) {
	return SnapshotWith(r, edgeID, nginxrender.DefaultProfile())
}

// SnapshotWith 用 profile.NginxConfDir 作为 tar 起始目录做快照。其它行为同 Snapshot。
//
// 清理 7 天前的旧备份，避免磁盘膨胀。清理失败不返回错误（仅写日志级影响）。
func SnapshotWith(r runner.Runner, edgeID uint, p nginxrender.Profile) (string, error) {
...
func Restore(r runner.Runner, backupPath string) error {
	return RestoreWith(r, backupPath, nginxrender.DefaultProfile())
}

// RestoreWith 同 Restore，但允许指定 Profile（多实例必需）。
func RestoreWith(r runner.Runner, backupPath string, p nginxrender.Profile) error {
	if backupPath == "" {
