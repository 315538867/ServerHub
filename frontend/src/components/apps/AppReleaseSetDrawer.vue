<template>
  <div class="arsd">
    <UiSection title="批量部署（Release Set）">
      <template #extra>
        ...
        <UiButton variant="ghost" size="sm" @click="emit('close')">收起</UiButton>
      </template>
      ...
      <UiCard padding="none">
        <NDataTable
          ...
          size="small"
        />
        <div v-if="!loading && rows.length === 0" class="arsd__empty">
...
async function reload() {
  loading.value = true
  try {
    rows.value = (await listAppReleaseSets(props.appId)) ?? []
  } catch {
    rows.value = []
  } finally {
    loading.value = false
...
async function doCreate() {
  creating.value = true
  try {
    await createAppReleaseSet(props.appId, {
...
async function runStream(mode: 'apply' | 'rollback') {
  ...
  try {
    await fn(props.appId, activeSet.value.id, {
      onEvent: applyEvent,
      signal: abortCtrl.signal,
    })
    activeSet.value = await getAppReleaseSet(props.appId, activeSet.value.id)
    await reload()
  } catch (e) {
