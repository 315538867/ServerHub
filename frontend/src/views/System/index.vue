<template>
  <div class="system-page">
    <div class="page-toolbar">
      <el-select v-model="selectedServerId" placeholder="选择服务器" style="width:220px" @change="onServerChange">
    ...
    <el-drawer
      ...
      :title="`服务日志 — ${svcLogsName}`"
      size="60%"
      direction="rtl"
      @closed="onSvcLogsClosed"
    >
...
async function loadFirewall() {
  ...
  loading.value = true
  try {
    const res = await getFirewallRules(selectedServerId.value)
...
async function loadCron() {
  ...
  loading.value = true
  try {
    cronJobs.value = await getCronJobs(selectedServerId.value)
  } finally { loading.value = false }
}
...
async function loadProcesses() {
  ...
  try {
    processes.value = await getProcesses(selectedServerId.value)
...
async function loadServices() {
  ...
  try {
    services.value = await getServices(selectedServerId.value)
...
async function confirmAddRule() {
  if (!selectedServerId.value || !ruleForm.value.port) return
  try {
    await addFirewallRule(selectedServerId.value, ruleForm.value)
...
async function delRule(row: FirewallRule) {
  if (!selectedServerId.value) return
  try {
    await deleteFirewallRule(selectedServerId.value, String(row.index))
...
function openCronAdd() {
  cronEditIndex.value = -1
  cronForm.value = { expr: '', cmd: '' }
...
async function confirmCron() {
  ...
  try {
    if (cronEditIndex.value === -1) {
      await addCronJob(selectedServerId.value, cronForm.value.expr, cronForm.value.cmd)
    } else {
      await updateCronJob(selectedServerId.value, cronEditIndex.value, cronForm.value.expr, cronForm.value.cmd)
    }
...
async function delCron(row: CronJob) {
  if (!selectedServerId.value) return
  try {
    await deleteCronJob(selectedServerId.value, row.index)
...
async function killProc(row: ProcessItem) {
  if (!selectedServerId.value) return
  try {
    await killProcess(selectedServerId.value, row.pid)
...
async function svcAction(row: ServiceItem, action: string) {
  if (!selectedServerId.value) return
  try {
    await serviceAction(selectedServerId.value, row.unit, action)
...
function initSvcLogs(unit: string) {
  if (!svcLogsEl.value || !selectedServerId.value) return
  svcLogsTerm?.dispose()
  ...
  fit.fit()
  svcLogsWs?.close()
  svcLogsWs = new WebSocket(serviceLogsWsUrl(selectedServerId.value, unit, auth.token))
