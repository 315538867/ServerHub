...
export function requestCertWsUrl(serverId: number, params: { domain: string; webroot?: string; email?: string }) {
...
export function renewCertWsUrl(serverId: number, certId: number) {
  return `${location.protocol === 'https:' ? 'wss:' : 'ws:'}//${location.host}/panel/api/v1/servers/${serverId}/ssl/certs/${certId}/renew`
}
